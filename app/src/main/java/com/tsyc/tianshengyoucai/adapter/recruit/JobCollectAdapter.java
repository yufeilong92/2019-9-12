 ...
 * @Purpose :职位列表
 */
public class JobCollectAdapter extends RecyclerView.Adapter<JobCollectAdapter.JobViewHolder> {
    private Context mContext;
    private List<JobCollectListVo.ResultBean.DataBean> mData;
    private final LayoutInflater mInflater;
    private int mType = 0;

    public JobCollectAdapter(Context mContext, List<JobCollectListVo.ResultBean.DataBean> mData) {
    ...
    public interface OnItemClickListener {
        void itemClick(int position, JobCollectListVo.ResultBean.DataBean vo);
    ...
    public void onBindViewHolder(@NonNull JobViewHolder holder, int position) {
        JobCollectListVo.ResultBean.DataBean vo = mData.get(position);
        JobCollectListVo.ResultBean.DataBean.PositionBean bean = vo.getPosition();
        JobCollectListVo.ResultBean.DataBean.PositionBean.CompanyBean company = bean.getCompany();

        JobCollectListVo.ResultBean.DataBean.PositionBean.BossBean boss = bean.getBoss();

        JobCollectListVo.ResultBean.DataBean.PositionBean.AddressInfoBean addressInfo = bean.getAddress_info();

        GmSelectBean jobType = bean.getJob_type();

        GmSelectBean work_experience = bean.getWork_experience();

        GmSelectBean education = bean.getEducation();


        holder.mIvInviteTitle.setText(bean.getPosition_name());
        holder.mIvInviteTag.setText(jobType.getDesc());
        holder.mIvInviteValues.setText(bean.getSalary().getDesc());
        if (company != null)
