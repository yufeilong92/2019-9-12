...
import com.tsyc.tianshengyoucai.ui.fragment.sub.EvaluateManageFragment;
...
public class EvaluateManageActivity extends BaseActivity {
    ...
    public void initView() {
        mTvTitle.setText(getString(R.string.text_evalute_manager));

    }

    @Override
    public void initData() {
        titles = new String[]{"五星", "四星", "三星", "二星", "一星"};
        initIndicator();
         List<Fragment> fragments = new ArrayList<>();
        fragments.clear();
        for (int i = 0; i < titles.length; i++) {
            EvaluateManageFragment orderFragment = new EvaluateManageFragment();
            Bundle bundle = new Bundle();
            bundle.putString("title", titles[i]);
            bundle.putString("position", String.valueOf(titles.length - i));
